...
export function DemolitionDerbyGameHub() {
  ...
  const renderMultiplayerLobby = () => {
    ...
    const isLocalReady = Boolean(localPlayer?.isReady && localPlayer?.role !== 'HOST');
    const hasMinPlayers = (roomState?.players?.length || 0) >= 2;
    const nonHostPlayers = roomState?.players?.filter((p) => p.role !== 'HOST' && p.userId !== roomState.hostId) || [];
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className={`text-[11px] font-bold flex items-center gap-1.5 transition-colors ${
                      isLocalReady
                        ? 'text-amber-500/60 cursor-not-allowed bg-amber-500/10 px-2 py-1 rounded-lg border border-amber-500/20'
                        : 'text-amber-400 hover:underline cursor-pointer'
                    }`}
                    title={isLocalReady ? 'Car selection locked while READY. Cancel ready to change car.' : 'Select or change vehicle'}
                  >
                    {isLocalReady ? <Lock className="w-3 h-3 text-amber-500" /> : <SlidersHorizontal className="w-3 h-3" />}
                    <span>{isLocalReady ? 'CAR LOCKED (UNREADY TO CHANGE)' : 'GARAGE (SELECT CAR)'}</span>
                  </button>
                </div>
                ...
                {isHost ? (
                  <div className="space-y-1">
                    ...
                    {!hasMinPlayers ? (
                      ...
                      <p className="text-[11px] text-amber-400/90 text-center font-semibold">
                        Waiting for all challengers to ready up before starting match.
                      </p>
                    ) : null}
                  </div>
                ) : (
                    ...
                    <div className="space-y-5">
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 bg-black/60 border border-white/10 rounded-2xl p-4 md:p-5">
                        ...
                        <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
                          ...
                          <div>
                            {isLocalReady ? (
                              <div className="w-full py-3 bg-amber-500/10 border border-amber-500/30 text-amber-400 font-bold text-center text-xs uppercase rounded-xl flex items-center justify-center gap-1.5">
                                <Lock className="w-4 h-4" />
                                <span>CAR LOCKED WHILE READY • UNREADY TO CHANGE</span>
                              </div>
                            ) : isModalEquipped ? (
